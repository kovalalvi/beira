...
class Unet_1D(nn.Module):
    ...
    def __init__(self,
        ...
        up_channels = [ch//decoder_reduce for ch in channels[:-1]] + channels[-1:]
        # channels
        
        self.upsample = UpConvBlock(scale=strides[-1],
                                    in_channels=up_channels[-1],
                                    out_channels=up_channels[-2],
                                    kernel_size=kernel_sizes[-1])
        
        self.upsample_blocks = nn.ModuleList([UpConvBlock(scale=strides[i],
                                                          in_channels=up_channels[i+1] + channels[i+1],
                                                          out_channels=up_channels[i],
                                                          kernel_size=kernel_sizes[i]) for i in range(self.model_depth-2, -1, -1)])
        
        
        self.conv1x1_one = nn.Conv1d(up_channels[0], self.n_channels_out, kernel_size=1, padding='same')
    ...
    def forward(self, x):
        ...
        for i in range(self.model_depth-1):
            merge = torch.cat([x, skips[i]], dim = 1)
            x = self.upsample_blocks[i](merge)
